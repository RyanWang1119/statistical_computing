...
chocolate_reviews_filtered <- chocolate %>%
  group_by(country_of_bean_origin) %>%  # group by origin
  summarize(rating_num = length(rating)) %>% filter(rating_num>=10) %>%  # at least 10 reviews
  filter(country_of_bean_origin != "Blend")  # remove blend

countries__part2 <- chocolate_reviews_filtered$country_of_bean_origin
chocolate_part2 <- chocolate %>% filter(country_of_bean_origin %in% countries__part2)
...
chocolate_reviews_with_continent <-chocolate_reviews_with_continent %>% 
  mutate(continent = ifelse(is.na(continent), fill_continent[country_of_bean_origin], continent))
```
...
ggplot(chocolate_50rating, aes(x = rating, y = cocoa_percent_numeric)) +
  ...
  # 1. Use points with an obnoxiously large size
  geom_point(size = 20, color = "yellow", alpha = 1) +
  
  # 2. Overuse color for no reason (random fill color)
  geom_point(aes(color = factor(review_date)), size = 15) +
  
  # 4. Remove the axis labels to confuse viewers
  labs(x = "", y = "") +
  
  # 5. Add an excessive, unreadable title and subtitle
  labs(title = "Ratings and Cocoa Percent\nDid you know that chocolate is sweet?", 
       subtitle = "Also, it's brown\nAnd people eat it") +
  
  # 6. Use a garish, eye-hurting color palette for background and text
  theme(
    plot.background = element_rect(fill = "orange"),
    panel.background = element_rect(fill = "purple"),
    plot.title = element_text(size = 30, face = "bold", color = "green"),
    plot.subtitle = element_text(size = 20, face = "italic", color = "red")
  ) +
  
  # 7. Rotate x-axis labels by 90 degrees unnecessarily
