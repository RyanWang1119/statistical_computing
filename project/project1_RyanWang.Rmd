...
output: 
  html_document:
    code_folding: "hide"
date: "2024-09-13"
>>>>>>> 6a2a5911ac9432a15be9ec3858181e5ac1389606
---
...
for (i in c(10, 15, 20, 25)) {
  p <- ggplot2::ggplot(data = chocolate, mapping = aes(rating)) + 
       ...
       labs(x="Rating", y = "Count",
            title = paste("Chocolate Ratings (", i, " bins)", sep = "")) 
  print(p)
}
...
=======
countries_with_60to70_percent <- chocolate_50rating %>% 
  group_by(country_of_bean_origin, percentage) %>%
  summarize(rating_mean_by_percent = mean(rating)) %>% 
  arrange(country_of_bean_origin, desc(rating_mean_by_percent)) %>%
  distinct(country_of_bean_origin, .keep_all = TRUE) %>% filter(percentage != ">=60 to <70%")

print(countries_with_60to70_percent)
>>>>>>> 6a2a5911ac9432a15be9ec3858181e5ac1389606
```
The second category, ">= 60 to <70%" has the highest rating on average. A few countries have disagreements with this result, such as "Belize"    "Blend"     "Brazil"    "Colombia"  "Ecuador"   "Mexico"    "Nicaragua".
